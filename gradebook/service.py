...
def add_student(name):
    ...
    student_obj = Student(
        new_id, name
    )  # me kriju objekt per mi validu & strukturu t'dhanat
...
def add_course(code, title):
    ...
    data = load_data()
    if any(
        c["code"] == code for c in data["courses"]
    ):  # me check nese vecse ekziston qaj kod
...
def enroll(student_id, course_code):
    ...
    data = load_data()
    if not any(
        s["id"] == student_id for s in data["students"]
    ):  # validation per a ekziston qaj student
        raise ValueError(f"No student with id {student_id}")
    if not any(
        c["code"] == course_code for c in data["courses"]
    ):  # validation per a ekziston qajo lande
        raise ValueError(f"No course with code {course_code}")
    if any(
        e["student_id"] == student_id and e["course_code"] == course_code
        for e in data["enrollments"]
    ):  # me kqyr nese vecse osht studenti i regjistrum n qat lande
    ...
    for e in data["enrollments"]:
        if e["student_id"] == student_id and e["course_code"] == course_code:
            enrollment_obj = Enrollment(
                e["student_id"], e["course_code"], e.get("grades", [])
            )
...
def list_students():
    ...
    data = load_data()
    return sorted(
        data["students"], key=lambda s: s["name"].lower()
    )  # me sort alfabetik pa dallim uppercase/lowercase - case-insensitive
